...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(input_size=embed_size, hidden_size=self.hidden_size, bias=True,
        ...
        self.target_vocab_projection = nn.Linear(in_features=self.hidden_size,
                                                 out_features=self.model_embeddings.target.weight.shape[0], bias=False)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        output, (henc, cenc) = self.encoder(
            nn.utils.rnn.pack_padded_sequence(X, source_lengths))
        enc_hiddens, sequence_length = nn.utils.rnn.pad_packed_sequence(output,
                                                                        batch_first=True)
        h0dec = self.h_projection(torch.cat((henc[0, :], henc[1, :]), 1))
        c0dec = self.c_projection(torch.cat((cenc[0, :], cenc[1, :]), 1))
        dec_init_state = (h0dec, c0dec)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, split_size_or_sections=1, dim=0):
            ...
            decoder_state, o_t, _ = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
